...
static const Dialect DEFAULT_DIALECT = Dialect::e310es;

const std::vector<Dialect>& allDialects() {
    static const std::vector<Dialect> ALL_DIALECTS{ { Dialect::e310es } };
    return ALL_DIALECTS;
}
    
#elif defined(Q_OS_MAC) 

static const Dialect DEFAULT_DIALECT = Dialect::e410;

const std::vector<Dialect>& allDialects() {
    static const std::vector<Dialect> ALL_DIALECTS{ Dialect::e410 };
    return ALL_DIALECTS;
}

#else

static const Dialect DEFAULT_DIALECT = Dialect::e450;

const std::vector<Dialect> & allDialects() {
    static const std::vector<Dialect> ALL_DIALECTS{ { Dialect::e450, Dialect::e410 } };
    return ALL_DIALECTS;
}
#endif
...
const std::string& dialectPath(Dialect dialect) {
    static const std::unordered_map<shader::Dialect, std::string> DIALECT_PATHS;
    static std::once_flag once;
    std::call_once(once, [&] {
        auto& map = const_cast<std::unordered_map<shader::Dialect, std::string>&>(DIALECT_PATHS);
#if defined(Q_OS_ANDROID) 
        map.insert({ shader::Dialect::e310es, "/310es/" });
#else 
        map.insert({ shader::Dialect::e410, "/410/" });
#if !defined(Q_OS_MAC)
        map.insert({ shader::Dialect::e450, "/450/" });
#endif
#endif
    });
    return DIALECT_PATHS.find(dialect)->second;
}
