...
void GeometryCache::buildShapes() {
    using namespace geometry;
    auto vertexBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::VertexBuffer);
    auto indexBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::IndexBuffer);
    // Cube
...
void GeometryCache::renderGrid(gpu::Batch& batch, const glm::vec2& minCorner, const glm::vec2& maxCorner,
    ...
    if (registered && (!_registeredGridBuffers.contains(id) || _lastRegisteredGridBuffer[id] != key)) {
        ...
        GridBuffer gridBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, sizeof(GridSchema), (const gpu::Byte*) &gridSchema);
...
void GeometryCache::updateVertices(int id, const QVector<glm::vec2>& points, const QVector<glm::vec4>& colors) {
    ...
    auto verticesBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::VertexBuffer);
    auto colorBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::VertexBuffer);
...
void GeometryCache::renderGlowLine(gpu::Batch& batch, const glm::vec3& p1, const glm::vec3& p2,
    ...
    if (!details.isCreated) {
        details.isCreated = true;
        details.uniformBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer);
...
graphics::MeshPointer GeometryCache::meshFromShape(Shape geometryShape, glm::vec3 color) {
    ...
    auto partBuffer = new gpu::Buffer(gpu::Buffer::UsageFlagBits::IndirectBuffer, sizeof(graphics::Mesh::Part), (gpu::Byte*)&part);
