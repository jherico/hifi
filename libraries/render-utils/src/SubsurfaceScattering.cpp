...
SubsurfaceScatteringResource::SubsurfaceScatteringResource() {
    ...
    _parametersBuffer = gpu::BufferView(std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, sizeof(Parameters), (const gpu::Byte*) &parameters));
...
void DebugSubsurfaceScattering::configure(const Config& config) {
    ...
    if (!_debugParams) {
        _debugParams = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, sizeof(glm::vec4), nullptr);
    }
