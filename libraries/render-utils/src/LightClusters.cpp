...
LightClusters::LightClusters() :
    _lightIndicesBuffer(std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer)),
...
uint32_t LightClusters::getNumClusters() const {
 ...
 void LightClusters::updateClusterResource() {
    ...
    if (numClusters != (uint32_t) _clusterGrid.size()) {
        ...
        _clusterGridBuffer._buffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, _clusterGridBuffer._size, (gpu::Byte*) _clusterGrid.data(), _clusterGridBuffer._size);
    }
    ...
    if (configListBudget != (uint32_t) _clusterContent.size()) {
        ...
        _clusterContentBuffer._buffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, _clusterContentBuffer._size, (gpu::Byte*) _clusterContent.data(), _clusterContentBuffer._size);
    }
}
