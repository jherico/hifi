...
graphics::MeshPointer Mesh::map(std::function<glm::vec3(glm::vec3)> vertexFunc,
    ...
    gpu::Buffer* resultVertexBuffer = new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, vertexSize, resultVertexData.get());
    ...
    gpu::Buffer* resultColorsBuffer = new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, colorSize, resultColorData.get());
    ...
    gpu::Buffer* resultNormalsBuffer = new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, normalSize, resultNormalData.get());
    ...
    gpu::Buffer* resultIndexesBuffer = new gpu::Buffer(gpu::Buffer::UsageFlagBits::IndexBuffer, indexSize, resultIndexData.get());
    ...
    result->setPartBuffer(gpu::BufferView(new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, parts.size() * sizeof(graphics::Mesh::Part),
...
MeshPointer Mesh::createIndexedTriangles_P3F(uint32_t numVertices, uint32_t numIndices, const glm::vec3* vertices, const uint32_t* indices) {
    ...
    // Vertex buffer
    mesh->setVertexBuffer(gpu::BufferView(new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, numVertices * sizeof(glm::vec3), (gpu::Byte*) vertices), gpu::Element::VEC3F_XYZ));
    ...
    if (numIndices < std::numeric_limits<uint16_t>::max()) {
        ...
        mesh->setIndexBuffer(gpu::BufferView(new gpu::Buffer(gpu::Buffer::UsageFlagBits::IndexBuffer, numIndices * sizeof(uint16_t), (gpu::Byte*) shortIndices), gpu::Element::INDEX_UINT16));
    } else {

        mesh->setIndexBuffer(gpu::BufferView(new gpu::Buffer(gpu::Buffer::UsageFlagBits::IndexBuffer, numIndices * sizeof(uint32_t), (gpu::Byte*) indices), gpu::Element::INDEX_INT32));
    }

    
    std::vector<graphics::Mesh::Part> parts;
    parts.push_back(graphics::Mesh::Part(0, numIndices, 0, graphics::Mesh::TRIANGLES));
    mesh->setPartBuffer(gpu::BufferView(new gpu::Buffer(gpu::Buffer::UsageFlagBits::VertexBuffer, parts.size() * sizeof(graphics::Mesh::Part), (gpu::Byte*) parts.data()), gpu::Element::PART_DRAWCALL));
