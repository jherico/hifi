...
scriptable::ScriptableMeshPointer GraphicsScriptingInterface::newMesh(const QVariantMap& ifsMeshData) {
    ...
    mesh->setIndexBuffer(buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::IndexBuffer, indices, gpu::Format::INDEX_INT32));
    mesh->setVertexBuffer(buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::VertexBuffer, vertices, gpu::Format::VEC3F_XYZ));
    if (normals.size()) {
        mesh->addAttribute(gpu::Stream::NORMAL, buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::VertexBuffer, normals, gpu::Format::VEC3F_XYZ));
    }
    if (colors.size()) {
        mesh->addAttribute(gpu::Stream::COLOR, buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::VertexBuffer, colors, gpu::Format::VEC3F_XYZ));
    }
    if (texCoords0.size()) {
        mesh->addAttribute(gpu::Stream::TEXCOORD0, buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::VertexBuffer, texCoords0, gpu::Format::VEC2F_UV));
    }
    QVector<graphics::Mesh::Part> parts = {{ 0, indices.size(), 0, topology }};
    mesh->setPartBuffer(buffer_helpers::newFromVector(gpu::Buffer::UsageFlagBits::IndirectBuffer, parts, gpu::Element::PART_DRAWCALL));
