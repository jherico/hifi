...
void FBXReader::buildModelMesh(FBXMesh& extractedMesh, const QString& url) {
    ...
    auto vertBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::VertexBuffer);
    ...
    // Finally, allocate and fill the attribBuffer interleaving the attributes as needed:
    {
        ...
        auto attribBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::VertexBuffer);
    ...
    if (parts.size()) {
        auto pb = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::IndirectBuffer);
