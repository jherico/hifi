...
void GLBackend::do_beginQuery(const Batch& batch, size_t paramOffset) {
    ...
    if (glquery) {
        PROFILE_RANGE_BEGIN(render_gpu_dx_detail, glquery->_profileRangeId, query->getName().c_str(), 0xFFFF7F00);
...
void GLBackend::do_endQuery(const Batch& batch, size_t paramOffset) {
    ...
    if (glquery) {
        ...
        PROFILE_RANGE_END(render_gpu_dx_detail, glquery->_profileRangeId);
