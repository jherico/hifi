...
void DrawBounds::run(const RenderContextPointer& renderContext,
    ...
    if (!_drawBuffer) {
        _drawBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::ResourceBuffer, sizeOfItemBound);
    }

    if (!_paramsBuffer) {
        _paramsBuffer = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::UniformBuffer, sizeof(vec4), nullptr);
    }
...
void DrawAABox::run(const render::RenderContextPointer& renderContext, const Inputs& box) {
    if (!box.isNull()) {
        ...
        if (!_cubeMeshIndices._buffer) {
            auto indices = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::IndexBuffer, sizeof(indexData), indexData);
