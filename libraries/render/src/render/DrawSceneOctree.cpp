...
void DrawItemSelection::run(const RenderContextPointer& renderContext, const ItemSpatialTree::ItemSelection& inSelection) {
    ...
    if (!_boundsBufferInside) {
        _boundsBufferInside = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::ResourceBuffer, sizeof(render::ItemBound));
    }
    if (!_boundsBufferInsideSubcell) {
        _boundsBufferInsideSubcell = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::ResourceBuffer, sizeof(render::ItemBound));
    }
    if (!_boundsBufferPartial) {
        _boundsBufferPartial = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::ResourceBuffer, sizeof(render::ItemBound));
    }
    if (!_boundsBufferPartialSubcell) {
        _boundsBufferPartialSubcell = std::make_shared<gpu::Buffer>(gpu::Buffer::UsageFlagBits::ResourceBuffer, sizeof(render::ItemBound));
    }
