...
class GL41Shader : public gl::GLShader {
    using Parent = gpu::gl::GLShader;

public:
    GL41Shader(const Shader& Shader) : Parent(Shader) {}

    GLShader* buildChildShader(const Shader& child) override {
        return GL41Shader::sync<GL41Shader>(child);
    }

    std::string getShaderHeader(Version version) const override {
        // GLSL version
        const std::string glslVersion { "#version 410 core" };
    ...
    void makeProgramBindings(ShaderObject& shaderObject) override {
        ...
        GLuint loc = glGetUniformLocation(glprogram, "transformObjectBuffer");
        if (loc >= 0) {
            glProgramUniform1i(glprogram, loc, TRANSFORM_OBJECT_SLOT);
...
bool GL41Backend::makeProgram(Shader& shader, const Shader::BindingSet& bindings) {
    return GL41Shader::makeProgram<GL41Shader>(shader, bindings);
}
